...
class TestIngestionEndpoints:
    ...
    def test_ingest_message_minimal(self, monkeypatch, tmp_path):
        ...
        response = client.post("/api/v1/ingest/message", json={"text": "Simple message"})
    ...
    def test_ingest_message_empty_text(self, monkeypatch, tmp_path):
        ...
        response = client.post("/api/v1/ingest/message", json={"text": ""})
    ...
    def test_ingest_message_missing_text(self, monkeypatch, tmp_path):
        ...
        response = client.post("/api/v1/ingest/message", json={"source": "api"})
    ...
    def test_ingest_message_integration(self, monkeypatch, tmp_path):
        ...
        response = client.post(
            "/api/v1/ingest/message",
            json={"text": "Integration test message", "source": "api", "metadata": {"test": True}},
        )
    ...
    def test_ingest_multiple_messages(self, monkeypatch, tmp_path):
        ...
        event_ids = []
        for msg in messages:
            response = client.post("/api/v1/ingest/message", json={"text": msg})
            assert response.status_code == 201
