"""Urgent reminder orchestration-path tests (M12)."""
...
class _FrozenDateTime(datetime):
    @classmethod
    def now(cls, tz=None):
        return cls(2026, 3, 2, 10, 2, 0)


@pytest.mark.asyncio
async def test_urgent_reminders_run_through_orchestration(monkeypatch, tmp_path):
    store = FileEventStore(data_dir=str(tmp_path / "events"))
    query = QueryService(store, db_path=tmp_path / "projections" / "urgent.db")
    task_service = TaskService(event_store=store, query_service=query)
    await task_service.ingest_task(
        TaskIngestRequest(
            title="Urgent reminder seed",
            source=SourceType.API,
            source_ref="m12-urgent-001",
    ...
    await scheduler.check_and_send_urgent_reminders(bot=object())
