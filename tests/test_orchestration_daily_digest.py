"""Daily digest orchestration-path tests (M12)."""
...
@pytest.mark.asyncio
async def test_daily_digest_runs_through_orchestration(monkeypatch, tmp_path):
    store = FileEventStore(data_dir=str(tmp_path / "events"))
    query = QueryService(store, db_path=tmp_path / "projections" / "daily.db")
    task_service = TaskService(event_store=store, query_service=query)
    await task_service.ingest_task(
        TaskIngestRequest(
            title="Daily digest seed",
            source=SourceType.API,
            source_ref="m12-daily-001",
